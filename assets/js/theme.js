...
// Local override: upstream cycles system -> light -> dark -> system, which makes
// the button a three-state control and leaves visitors unsure what the third
// state does. Flip between light and dark only.
//
// The site still *starts* in system mode, so a first-time visitor gets whatever
// their OS prefers. Switching off determineComputedTheme() rather than the
// stored setting means the first click always flips what is actually on screen,
// instead of jumping to light for someone already viewing dark via system.
